...
const emit = <K extends keyof ListenerMap>(ev: K, payload?: any) => {
  (L[ev] || []).forEach(fn => (fn as any)(payload));
};
...
const WebStub: SoccerBluetoothPlugin = {
  ...
  async requestLEScan() {
    ...
    // Emit devices immediately (for UIs that check synchronously)…
    const devs = mkDevices();
    bufferedScan = devs;
    lastDevice = devs[0];
    devs.forEach(d => emit('scanResult', d));

    // …and again shortly after (for late listeners)
    setTimeout(() => devs.forEach(d => emit('scanResult', d)), 400);

    // Some flows wait for scanStopped
    setTimeout(() => emit('scanStopped'), 1200);

    // If UI never calls connect(), auto-connect so demo data shows up
  ...
  async addListener(eventName: any, listenerFunc: any) {
    ...
    // Replay buffered devices for late subscribers
...
/* =========================
   Export: native on iOS, stub on web
   ========================= */
